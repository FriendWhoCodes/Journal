...
function buildPrompt(priorities: Priority[], identity: Identity, year: number = new Date().getFullYear()): string {
  ...
  return `You are a wise life coach reviewing someone's ${year} annual blueprint. Provide thoughtful, personalized feedback.
...
function generatePlaceholderFeedback(priorities: Priority[], identity: Identity, year: number = new Date().getFullYear()): AIFeedbackResult {
  ...
  return {
    ...
    overallWisdom: `Your ${year} Blueprint reflects genuine intention and self-awareness. The fact that you've taken the time to define your priorities and set goals puts you ahead of most people.\n\nRemember: a plan is a living document. The value isn't in perfection — it's in having a direction. Review this blueprint monthly, celebrate small wins, and don't be afraid to adjust as you grow. The person you'll be at the end of ${year} will be grateful you took this step today.\n\nStay consistent, stay patient, and trust the process.`,
  };
}
...
export async function generateAIFeedback(
  ...
  if (!AI_API_KEY) {
    ...
    return generatePlaceholderFeedback(priorities, identity, year);
  }

  try {
    const prompt = buildPrompt(priorities, identity, year);
